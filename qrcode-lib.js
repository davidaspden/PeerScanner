/**
 * qrcode-lib.js - Lightweight, self-contained, synchronous QR Code Generator
...
(function (root, factory) {
    if (typeof define === 'function' && define.amd) {
        ...
        root.QRCode = root.QRCode || {};
        root.QRCode.toCanvas = function (canvas, text, options, cb) {
            try {
                root.QRCodeLib.drawToCanvas(canvas, text, options);
                if (typeof cb === 'function') cb(null, canvas);
    ...
    // QR Code implementation based on Kazuhiko Arase's algorithm
    ...
    function QRPolynomialClass(num, shift) {
    ...
    QRPolynomialClass.prototype = {
        ...
        multiply: function (e) {
            ...
            return new QRPolynomialClass(num, 0);
        },
        mod: function (e) {
            ...
            return new QRPolynomialClass(num, 0).mod(e);
        }
    };
    ...
    var QRUtil = {
        ...
        getErrorCorrectionPolynomial: function (errorCorrectionLength) {
            var a = new QRPolynomialClass([1], 0);
            for (var i = 0; i < errorCorrectionLength; i++) {
                a = a.multiply(new QRPolynomialClass([1, QRPolynomial.gexp(i)], 0));
            }
            return a;
        }
    };
    ...
    QRCodeModel.createBytes = function (buffer, rsBlocks) {
        ...
        for (var r = 0; r < rsBlocks.length; r++) {
            ...
            var rawPoly = new QRPolynomialClass(dcdata[r], rsPoly.getLength() - 1);
    ...
    function drawToCanvas(canvas, text, options) {
        options = options || {};
        var ecLevel = 0; // 'M'
        if (options.errorCorrectionLevel === 'L') ecLevel = 1;
        else if (options.errorCorrectionLevel === 'Q') ecLevel = 3;
        else if (options.errorCorrectionLevel === 'H') ecLevel = 2;
        ...
        var width = options.width || canvas.width || 256;
        var height = options.height || canvas.height || 256;
        canvas.width = width;
        canvas.height = height;

        var margin = options.margin !== undefined ? options.margin : 2;
        var cellSize = Math.floor(Math.min(width, height) / (moduleCount + margin * 2));
